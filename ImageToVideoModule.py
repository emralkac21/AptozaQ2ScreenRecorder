...
# Ana uygulama temasıyla uyumlu renkler
    ...
    def _open_effects_window(self, idx):
        ...
        def schedule_preview(*args):
            ...
            if self._preview_timer is not None:
                win.after_cancel(self._preview_timer)
            self._preview_timer = win.after(400, generate_preview) # 400ms bekler, kasmayı önler
        ...
        def _run_ffmpeg_preview(current_eff):
            ...
            if r.returncode == 0 and os.path.exists(tmp_img):
                try:
                    ...
                    pil_image.thumbnail((400, 400)) # Görüntüyü ekrana sığacak şekilde küçült
                    ctk_img = ctk.CTkImage(light_image=pil_image, dark_image=pil_image, size=pil_image.size)
                    win.after(0, lambda: self.preview_lbl.configure(image=ctk_img, text=""))
                except Exception as e:
                    win.after(0, lambda: self.preview_lbl.configure(text="Resim Yüklenemedi.", image=""))
            else:
                win.after(0, lambda: self.preview_lbl.configure(text="Hata oluştu.", image=""))
            ...
            try:
                ...
                for i, img in enumerate(self.images):
                    ...
                    cmd = ["ffmpeg", "-y", "-loop", "1", "-t", str(dur), "-i", img["path"],
                           "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30", "-vf", vf_string, clip_path]
                ...
                r2 = subprocess.run(cmd_merge, capture_output=True, timeout=60)
                if r2.returncode == 0:
