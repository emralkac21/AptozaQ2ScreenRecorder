...
class RecordingEngine:
    ...
    def _loop_screen(self):
        ...
        if FFMPEG:
            ...
            self._ffmpeg_proc = subprocess.Popen(ffcmd, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
                ...
                if frames_to_write > 0:
                    ...
                    if pip_active:
                        ...
                        if ret and wf is not None:
                            ...
                            _m = 20
                            _pp = self.pip_pos
                            if   _pp == "top-left":     ox, oy = _m,              _m
                            elif _pp == "top-right":    ox, oy = W - pb_w - _m,   _m
                            elif _pp == "bottom-left":  ox, oy = _m,              H - pb_h - _m
                            else:                       ox, oy = W - pb_w - _m,   H - pb_h - _m
    ...
    def _wm_pos(self, w, h, W, H):
        ...
        elif p == "top-right":    return (W - w - m, m)
        elif p == "bottom-left":  return (m, H - h - m)
        elif p == "center":       return ((W - w) // 2, (H - h) // 2)
        else:                     return (W - w - m, H - h - m)
    ...
    def _loop_audio(self):
        ...
        try:
            ...
            while self.recording:
                ...
                try: self._audio_f.append(st.read(CHUNK, exception_on_overflow=False))
                except: break
    ...
    def _save_wav(self):
        try:
            ...
            wf.writeframes(b"".join(self._audio_f)); wf.close()
    ...
    def hide(self):
        try:
            ...
            if hasattr(tk, '_default_root') and tk._default_root:
                tk._default_root.tool_window.wm_transient("")
        ...
        if self.tool == "rectangle": self._cur = self.canvas.create_rectangle(x0, y0, x1, y1, outline=c, width=t)
            ...
            with self.engine.draw_lock:
                if self.engine.draw_layer:
                    ...
                    self.engine.draw_layer.paste(patch, (x1-r, y1-r))
...
class VideoEditor:
    ...
    def trim(inp, out, t0, t1, cb=None):
        def _run():
            try:
                r = subprocess.run(["ffmpeg", "-y", "-ss", str(t0), "-i", inp, "-to", str(t1 - t0), "-c:v", "libx264", "-crf", "18", "-preset", "medium", "-c:a", "aac", "-b:a", "192k", out], capture_output=True, timeout=300)
    ...
    def merge(paths, out, cb=None):
        def _run():
            try:
                ...
                r = subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", lst, "-c:v", "libx264", "-crf", "18", "-preset", "medium", "-c:a", "aac", "-b:a", "192k", out], capture_output=True, timeout=600)
                try: os.remove(lst)
                except: pass
                if cb: cb(r.returncode == 0, out)
            except Exception as e:
    ...
    def convert(inp, out, fmt="mp4", cb=None):
        def _run():
            ...
            try:
                r = subprocess.run(["ffmpeg", "-y", "-i", inp] + extra + [out], capture_output=True, timeout=600)
                if cb: cb(r.returncode == 0, out)
            except Exception as e:
                if cb: cb(False, str(e))
        threading.Thread(target=_run, daemon=True).start()
    ...
    def get_duration(path):
        try:
            r = subprocess.run(["ffprobe", "-v", "quiet", "-show_entries", "format=duration", "-of", "csv=p=0", path], capture_output=True, text=True, timeout=10)
...
class AudioEditor:
    ...
    def trim(inp, out, t0, t1, cb=None):
        ...
        def _run():
            try:
                ...
                codec_args = codec_map.get(ext, ["-c:a", "copy"])
    ...
    def convert(inp, out, fmt="mp3", bitrate="192k", cb=None):
        ...
        def _run():
            ...
            codec_args = codec_map.get(fmt, ["-c:a", "copy"])
            try:
                r = subprocess.run(
...
class VideoTimelineEditor(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        tb = ctk.CTkFrame(left, fg_color=CARD, corner_radius=8)
        tb.pack(fill="x", padx=(0, 6), pady=4)
        btn_cfg = dict(height=32, font=ctk.CTkFont("Arial", 12))
        ctk.CTkButton(tb, text="📂 Video Ekle", width=110, fg_color=ACCENT2, hover_color="#1a4472", command=self._import_video, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🎵 Ses Ekle", width=110, fg_color="#1a6b66", hover_color=BORDER, command=self._import_audio, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🖺 Metin Ekle", width=110, fg_color="#ff9500", hover_color="#7c3aed", command=self._add_text_overlay, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🎬 Geçiş Efekti", width=110, fg_color="#7c3aed", hover_color="#5b21b6", command=self._open_transition_picker, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🔪 Böl", width=60, fg_color="#af8f00", hover_color="#7f1d1d", command=self._split_selected, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🎵 Sesi Ayır", width=100, fg_color="#df1962", hover_color=BORDER, command=self._extract_audio, **btn_cfg).pack(side="left", padx=3, pady=6)
        self.btn_mute_clip = ctk.CTkButton(tb, text="🔇 Sesi Sil", width=90, fg_color="#386293", hover_color="#5a1a1a", command=self._toggle_mute_selected, **btn_cfg)
        self.btn_mute_clip.pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="🚮 Sil", width=60, fg_color=RED, hover_color="#7f1d1d", command=self._delete_selected, **btn_cfg).pack(side="left", padx=3, pady=6)
        ctk.CTkButton(tb, text="💾 Dışa Aktar", width=110, fg_color=YELLOW, hover_color="#a07010", text_color="#000", font=ctk.CTkFont("Arial", 12, "bold"), command=self._export).pack(side="right", padx=3, pady=6)
        
        tl_card = ctk.CTkFrame(left, fg_color=CARD, corner_radius=8)
        tl_card.pack(fill="x", padx=(0, 6), pady=4)
        tl_hdr = ctk.CTkFrame(tl_card, fg_color="transparent")
        tl_hdr.pack(fill="x", padx=8, pady=(6, 2))
        ...
        self.tl_canvas = tk.Canvas(tl_wrap, bg=CARD2, height=126, highlightthickness=1, highlightbackground=BORDER)
        ...
        # ── Sağ Panel: Sekmeli Özellikler ─────────────────────────────────
        right_frame = ctk.CTkFrame(body, fg_color=CARD, corner_radius=10, width=380)
        ...
        ctk.CTkFrame(tab_clip, height=1, fg_color=BORDER).pack(fill="x", padx=8, pady=6)
        ctk.CTkLabel(tab_clip, text="⚡  Hız (Speed)",
                     font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2).pack(anchor="w", padx=8, pady=(2, 2))
        speed_f = ctk.CTkFrame(tab_clip, fg_color=CARD, corner_radius=6)
        ...
        ctk.CTkFrame(tab_clip, height=1, fg_color=BORDER).pack(fill="x", padx=8, pady=6)
        ctk.CTkLabel(tab_clip, text="📐  Boyut / Ölçek",
                     font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2).pack(anchor="w", padx=8, pady=(2, 2))
        scale_f = ctk.CTkFrame(tab_clip, fg_color=CARD, corner_radius=6)
        ...
        self.audio_list_frame = ctk.CTkScrollableFrame(tab_audio, fg_color=CARD, corner_radius=6, height=160)
    ...
    def _update_overlay_list(self):
        ...
        for ov in self.text_overlays:
            ...
            ctk.CTkLabel(r, text=f"📝 {ov.text[:16]}  ({ov.start_time:.1f}s→{ov.end_time:.1f}s)", font=ctk.CTkFont("Arial", 10), text_color=TXT).pack(side="left", padx=6, pady=4)
            ctk.CTkButton(r, text="🗑", width=28, height=24, fg_color=RED, command=lambda o=ov: self._remove_overlay(o)).pack(side="right", padx=4, pady=4)
    ...
    def _update_audio_list(self):
        ...
        for at in self.audio_tracks:
            ...
            ctk.CTkLabel(r, text=f"🎵 {at.name[:18]}", font=ctk.CTkFont("Arial", 10), text_color=TXT).pack(side="left", padx=6, pady=4)
            ctk.CTkButton(r, text="🗑", width=28, height=24, fg_color=RED, command=lambda a=at: self._remove_audio(a)).pack(side="right", padx=4, pady=4)
            ...
            try:
                ...
                for i, clip in enumerate(sorted_clips):
                    ...
                    for ov in self.text_overlays:
                        ...
                        if rel_e > 0 and rel_s < clip_dur:
                            ...
                            safe = ov.text.replace("\\","\\\\").replace("'","\\'").replace(":","\\:")
                            hx   = ov.color.lstrip("#")
                            v_filters.append(
                                f"drawtext=text='{safe}':fontsize={ov.size}:"
                ...
                if len(processed) == 1:
                    ...
                    if not has_fx:
                        ...
                        rx = subprocess.run(xfade_cmd, capture_output=True, timeout=1200)
                        if rx.returncode != 0:
                ...
                if self.audio_tracks and os.path.exists(final_vid):
                    ...
                    r3 = subprocess.run(mix_cmd, capture_output=True, timeout=600)
                    if r3.returncode != 0:
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        logo = ctk.CTkLabel(self.sidebar, text="AptozaQ2", font=ctk.CTkFont("Arial", 24, "bold"), text_color=TXT); logo.pack(pady=(10, 4), padx=16)
        ctk.CTkLabel(self.sidebar, text="V1.1.0", font=ctk.CTkFont("Arial", 11), text_color=TXT2).pack()
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=5)
        self._nav_btns = {}
        ...
        for icon, label, cmd in nav_items:
            btn = ctk.CTkButton(self.sidebar, text=f"  {icon}  {label}", anchor="w", height=36, font=ctk.CTkFont("Arial", 14), fg_color="#1f538d", hover_color=CARD2, text_color=TXT2, command=cmd)
            btn.pack(fill="x", padx=8, pady=2); self._nav_btns[label] = btn
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=5, pady=6)
        self.btn_show_tools = ctk.CTkButton(self.sidebar, text="🛠️ Araç ve Zoom Paneli", height=30, fg_color=CARD2, hover_color="#FF0000", command=self.tool_window.show)
        self.btn_show_tools.pack(fill="x", padx=10, pady=(0, 10))
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=10)
        ctk.CTkLabel(self.sidebar, text="📸 Ekran Görüntüsü", font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2).pack(pady=(4, 4))
        self.ss_fmt_var = ctk.StringVar(value="png")
        ctk.CTkOptionMenu(self.sidebar, variable=self.ss_fmt_var, values=["png", "jpg", "bmp", "webp", "tiff"], width=100).pack(padx=5, pady=(0, 4))
        ctk.CTkButton(self.sidebar, text="📷  Ekran Görüntüsü Al", height=34, fg_color=ACCENT2, hover_color=BORDER, font=ctk.CTkFont("Arial", 12, "bold"), command=self._take_screenshot).pack(fill="x", padx=10, pady=(0, 8))
        self.lbl_status_side = ctk.CTkLabel(self.sidebar, text="● Hazır", font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2); self.lbl_status_side.pack(pady=4)
        self.lbl_timer_side  = ctk.CTkLabel(self.sidebar, text="00:00:00", font=ctk.CTkFont("Courier", 26, "bold"), text_color=TXT); self.lbl_timer_side.pack(pady=4)
        self.btn_start = ctk.CTkButton(self.sidebar, text="⏺  Kayıt Başlat", height=36, fg_color="#1f538d", hover_color="#FF0000", font=ctk.CTkFont("Arial", 13, "bold"), command=self._start_recording)
        self.btn_start.pack(fill="x", padx=10, pady=(8, 3))
        self.btn_pause = ctk.CTkButton(self.sidebar, text="⏸  Duraklat", height=36, fg_color="#1f538d", hover_color="#FF6F00", text_color="#000", font=ctk.CTkFont("Arial", 13, "bold"), state="disabled", command=self._pause_resume)
        self.btn_pause.pack(fill="x", padx=10, pady=3)
        self.btn_stop = ctk.CTkButton(self.sidebar, text="⏹  Durdur", height=36, fg_color="#1f538d", hover_color="#CF35AE", font=ctk.CTkFont("Arial", 13, "bold"), state="disabled", command=self._stop_recording)
        self.btn_stop.pack(fill="x", padx=10, pady=3)
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=5)
        self.draw_var = ctk.BooleanVar(value=False)
        self.btn_draw_toggle = ctk.CTkSwitch(self.sidebar, text="Ekranda Çizim Modu", variable=self.draw_var, command=self._toggle_draw, font=ctk.CTkFont("Arial", 13, "bold"), text_color="#ffffff")
        self.btn_draw_toggle.pack(padx=16, pady=4)
        ctk.CTkFrame(self.sidebar, height=1, fg_color=BORDER).pack(fill="x", padx=10, pady=(5, 4))
        ctk.CTkLabel(self.sidebar, text="🎨 Tema", font=ctk.CTkFont("Arial", 12, "bold"), text_color=TXT2).pack(pady=(0, 2))
        self.appearance_mode_menu = ctk.CTkOptionMenu(self.sidebar, values=["Koyu", "Açık", "Sistem"], command=self._change_appearance_mode, width=150)
    ...
    def _start_recording(self):
        self._apply_engine_settings(); self.engine.start()
    ...
    def _stop_recording(self):
        ...
        self.btn_start.configure(state="normal"); self.btn_pause.configure(state="disabled", text="⏸  Duraklat"); self.btn_stop.configure(state="disabled")
    ...
    def _set_scheduler(self):
        ...
        if start_dt < now: start_dt = start_dt.replace(day=now.day+1)
        if end_dt < start_dt: end_dt = end_dt.replace(day=start_dt.day+1)
